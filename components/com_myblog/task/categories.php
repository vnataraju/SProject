...
{	
	...
	function display($styleid = '', $wrapTag = 'div')
	{
		...
		if(empty($styleid))
		{
			myAddPageTitle( JText::_('COM_MY_SHOW_TAGS_TITLE') );
		}
		...
		$categories = "";
		$categories .= "<div class='mbCommonWrap' id='mbCatTagWrap'>";
		if ($categoriesArray)
		{
			foreach ($categoriesArray as $category)
			{
				...
				else
				{
					if(isset($blogger) && !empty($blogger))
					{
						$categories .= "<a class=\"$catclass mbTags\" href=\"" . JRoute::_("index.php?option=com_myblog&category=" . $tagSlug . "&blogger=$blogger&Itemid=$mbItemid") . "\">$catname</a>";
					}
					else
					{
						$categories .= "<a class=\"$catclass mbTags\" href=\"" . JRoute::_("index.php?option=com_myblog&task=tag&category=" . $tagSlug . "&Itemid=$mbItemid") . "\">$catname</a>";
					}
		...
		$content .= "</{$wrapTag}>";
		$categories .= "<div class='clear'></div></div>";
		$pageheader = '<h2 class="componentheading">'.JText::_('COM_MY_TAGS_PAGE_TITLE') .'</h2>';
		$content = '<div class="mbCommonWrap">'.$pageheader.$content.'</div>';
		return $content;
	}
