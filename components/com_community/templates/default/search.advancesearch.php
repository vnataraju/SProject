			...
			action: {
				...
				addCriteria: function ( ) {
					...
					criteria +='<div id="criteria'+keynum+'" class="criteria-row">';
						criteria +='<div id="removelink'+keynum+'"  style="float:left; margin:0 6px 0 2px; padding-top:5px; width: 20px">';
									...
									?>		
											case "<?php echo $field->fieldcode; ?>":
												<?php if ($field->type == 'checkbox') { ?>
													...
													{
													?>
														list += '<div style="padding:0 10px 0 0;float: left;"><input type="checkbox" class="inputbox" name="value'+id+'[]" value="<?php echo addslashes(JString::trim($data)); ?>"><?php echo JText::_(addslashes(JString::trim($data))); ?></input></div>';	
													<?php
													}
													?>
													list	+= '<div class="clr"></div>';													
				...
				addAltInputField: function(type, id) {
					...
					if(cond == "between"){
						if(type=='birthdate' || type=='date'){
							...
							inputField  = '<input class="inputbox" type="text" name="value'+id+'_2" id="value'+id+'_2" style="width:125px; margin-right:4px" value=""/>';
						}
					}else{
						inputField = '';
					}
				...
				calendar: function(type, id) {
					var inputField;
					if(type=='birthdate' || type=='date'){
						inputField  = '<a onclick="jsAdvanceSearch.action.toggleAgeSearch('+id+',1);" href="javascript:void(0);" title="<?php echo JText::_('COM_COMMUNITY_ADVSEARCH_AGE_TITLE') ?>"> <?php echo JText::_('COM_COMMUNITY_ADVSEARCH_DATE'); ?> </a><input class="inputbox" type="text" name="value'+id+'" id="value'+id+'" style="width:100px; margin-right:4px" value="" title="'+this.dateFormatDesc+'" readonly="false"/><img class="calendar" src="<?php echo rtrim( JURI::root(), "/"); ?>/components/com_community/assets/calendar.png" alt="calendar" id="value'+id+'_img" />';
					}else{
				...
				toggleAgeSearch: function(id,mode) {
					var cond = joms.jQuery('#condition'+id).val();
					if(mode == 1){
						inputField  = '<a onclick="jsAdvanceSearch.action.toggleAgeSearch('+id+',0);" href="javascript:void(0);" title="<?php echo JText::_('COM_COMMUNITY_ADVSEARCH_DATE_TITLE') ?>"> <?php echo JText::_('COM_COMMUNITY_ADVSEARCH_AGE'); ?></a><input class="inputbox" type="text" name="value'+id+'" id="value'+id+'" style="width:100px; margin-right:4px" value="" />';
						joms.jQuery('#valueinput'+id).html(inputField);
						if(cond == "between"){
...
<form name="jsform-search-advancesearch" action="<?php echo CRoute::getURI(); ?>" method="GET">
	...
	<div id="optionContainer">
		<div class="criteria-option-top">
			<a class="add" href="javascript:void(0);" onclick="jsAdvanceSearch.action.addCriteria();"><?php echo JText::_("COM_COMMUNITY_ADD_CRITERIA"); ?></a>
		</div>
		<div class="criteria-option-btm">
			...
			<label class="lblradio" style="padding-right: 20px;"><input type="checkbox" name="avatar" id="avatar" style="margin-right: 5px;" value="1" class="radio"<?php echo ($avatarOnly) ? ' checked="checked"' : ''; ?>><?php echo JText::_('COM_COMMUNITY_EVENTS_AVATAR_ONLY'); ?></label>
