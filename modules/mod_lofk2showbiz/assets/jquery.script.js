...
(function($,undefined){	
	...
	$.fn.extend({
		...
		var defaults = {	
			...
			options = $.extend({}, $.fn.lofservices.defaults, options);
				...
				if (opt.touchenabled=="on")
						...
						top_container.swipe( {data:top_container, 
										swipeLeft:function() 
												{ 
													var top=$(this);
													var toolbar = top.find('.showbiz-toolbar');
		...
		///////////////////////////////
					...
					function sideButtons(opt,onoff) {
									...
									var toolbar = opt.topcontainer.find('.showbiz-toolbar');																										
									if (onoff=="none") {
					...
					function openReadMorePart(slide,opt,dir) {
										...
										opt.top.animate({'top':(opt.height*1.5)+"px"},{duration:opt.duration,queue:false});										
										...
										if (dir==2) {
												...
												pm.animate({'left':'0px',"opacity":'1.0'},{duration:opt.duration,queue:false});		
										} else {																				
										if (dir==1) {
												pm.css({'left':"150px","opacity":'0.0'});
												pm.animate({'left':'0px',"opacity":'1.0'},{duration:opt.duration,queue:false});	
												...
												pm.animate({'top':'0px'},{duration:1000,queue:false});		
										}}
										...
										top.find('ul:first >li .slide').each(function(i) {
											...
											if (direction==0) {
												...
												if ($this.data("id") == opt.maxitem-1) { 													
													...
													$this.cssAnimate({'left':(1*minusslide)+"px"},{duration:opt.durationSlide,queue:false});
												} else {
													...
													if ($this.position().left<0) {
														$this.cssAnimate({'left':overslide+"px"},{duration:1});
															...
															setTimeout(function() {
																$this.cssAnimate({'left':opt.positionArray[$this.data('id')]+"px"},{duration:opt.durationSlide,queue:false});
																},100);//+$this.data('id')*10);
													}
												}
											} else {
												...
												if ($this.data("id") == opt.slideAmount) { 													
													$this.cssAnimate({'left':overslide+"px"},{duration:opt.durationSlide,queue:false});
												} else {
					...
					function prepareSlides(top,opt) {
						...
						top.find('ul:first > li').each(function(i) {
							...
							slide.find('.page-more').each(function() {
								...
								slide.find('.morebutton').click(function() {openReadMorePart(slide,opt);return false;});																							
							...
							if (slide.data('pagemore').length>0)
								slide.find('.imgholder').click(function() {openReadMorePart(slide,opt);return false;});
								...
								function () {
									...
									$this.parent().parent().find('li').each(function(i) {										
										...
										if (!$this.find('.slide').hasClass("mouseover")) {
											...
											$this.find('.slide .imgholder .bw').animate({'opacity':'1.0'},{duration:200,queue:false});										
												...
												$this.find('.slide *').each(function() {
															var $this=$(this);
																...
																if (!$this.hasClass("buttonlight") && !$this.hasClass('imgholder'))
																	$this.animate({'opacity':'0.5'},{duration:200,queue:false});
																if ($this.hasClass("thumb"))
																	$this.animate({'opacity':'0.0'},{duration:200,queue:false});
											...
											$this.find('.slide .imgholder .bw').stop();
											$this.find('.slide *').stop();
											$this.find('.slide  .imgholder .bw').animate({'opacity':'0.0'},{duration:200,queue:false});
											if ($.browser.msie && $.browser.version < 8) {											
