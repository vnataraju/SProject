...
class MYMediaBrowser extends MYMedia{
	...
	function upload($file, $resize = false, $uniqeName = false, $quickPost=false)
	{
		global $_MY_CONFIG;
		...
		$fileName	= JString::strtolower($file['name']);
                ...
                $newFilename = $uniqeName==false?$file['name']:$uniq;
		$save		= $this->media . '/' . $newFilename;
		...
		// check the extension
		if($quickPost==true){
		    $extAllowed = 'png,jpg,gif,jpeg';
		}else{
		    $extAllowed = $_MY_CONFIG->get('allowedUploadFileType');
		}
		
		$extension = explode('.',$file['name']);
		$extension = strtolower($extension[count($extension)-1]);

		if(strpos($extAllowed,$extension)===false){
		    $error	= sprintf(JText::_('COM_MY_MUST_HAVE_EXT'), $extAllowed);
		}
		...
		*/
		...
		{
			$error	= JText::_('COM_MY_FILE_TOO_LARGE');
		}

		if(file_exists($save))
		{
			$error	= JText::_('COM_MY_FILE_EXIST');
		}

		if(empty($error))
		{
			if(!JFile::upload($file['tmp_name'], $save) )
			{
				$error	= sprintf(JText::_('COM_MY_ERROR_MOVING_FILE'), $file['tmp_name']);
			}
		}
		...
		if( empty( $error ) )
		{
			...
			$msg	= sprintf(JText::_('COM_MY_UPLOAD_FILE_SUCCESS'), $file['name']);

			// For security reason, we force to remove all uploaded file
			JFile::delete($file['tmp_name']);
		}

		return array('error' => $error, 'msg' => $msg , 'source' => $this->mediaURI . '/' . $newFilename );
	}
	...
	{
		return JFile::write( $location , $content );
	}
		...
		if(array_key_exists($extension, $this->_documents)){
			...
			if(@list($width, $height) = getimagesize($filepath)){
				...
				if($width > $this->_thumbnail['width']){
					...
					$image		= rtrim( JURI::root() , '/' ) . '/index.php?option=com_myblog&task=thumb&maxwidth=' . $this->_thumbnail['width'] . '&fn=' . urlencode($filepath);
				}
			}
