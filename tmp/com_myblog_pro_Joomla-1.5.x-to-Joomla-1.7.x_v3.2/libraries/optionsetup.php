...
class MYOptionSetup
{
  ...
  //       $html .= '</td></tr></table>';
		...
		$html = '<table width="100%" border="0" cellspacing="10" cellpadding="0" class="mytable cfgdesc">';
		foreach($this->data  as $cfg)
		{
		    $anchor = strtolower(str_replace(" ","_",$cfg[0]['title']));
			$html .= '<th colspan="2"><a name="' . $anchor .'"></a>'.$cfg[0]['title'].'</th>';
			for($i = 1; $i < count($cfg); $i++)
			{
				$text = "";
				$html .= '<tr>';
				$html .= '<td width="25%" valign="top"><label class="cfgdesc" for="'.$cfg[$i]['name'].'">'.$cfg[$i]['title'].'<label>';
				...
				$html .= $text.'<div class="cfgdesc" >'.$cfg[$i]['desc'].'</div></td></tr>';
