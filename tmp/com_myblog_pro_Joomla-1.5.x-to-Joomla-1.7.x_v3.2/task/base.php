	...
	function _header()
	{
		...
		if ($_MY_CONFIG->get('overrideTemplate'))
		{
			...
			if( JFile::exists($myCustomTplStyle) )
			{
				...
				//$mainframe->addCustomHeadTag($style);
                                $document->addStyleSheet( rtrim( JURI::root() , '/' ) . '/templates/' .$mainframe->getTemplate() .'/html/com_myblog/css/template_style.css');
		...
		else
		{
			if(JFile::exists(MY_TEMPLATE_PATH . "/" . $_MY_CONFIG->get('template') . "/css/template_style.css"))
			{
				//$style = '<link rel="stylesheet" type="text/css" href="' . rtrim( JURI::root() , '/' ) . '/components/com_myblog/templates/' . $_MY_CONFIG->get('template') . "/css/template_style.css" . '"/>';
				//$mainframe->addCustomHeadTag($style);
				$document->addStyleSheet( rtrim( JURI::root() , '/' ) . '/components/com_myblog/templates/' . $_MY_CONFIG->get('template') . "/css/template_style.css");
			}
		}
		...
		if ( JFile::exists(MY_TEMPLATE_PATH . "/" . $_MY_CONFIG->get('template') . "/css/IE6.css" ) )
		{
			...
			$style .= '<![endif]-->';			
			//$mainframe->addCustomHeadTag($style);
			$document->addStyleSheet( rtrim( JURI::root() , '/' ) . '/components/com_myblog/templates/' . $_MY_CONFIG->get('template') . "/css/IE6.css");
		}
		...
		if ( JFile::exists(MY_TEMPLATE_PATH . "/" . $_MY_CONFIG->get('template') . "/css/IE7.css" ) )
		{
			...
			$style .= '<![endif]-->';	
			//$mainframe->addCustomHeadTag($style);
			$document->addStyleSheet( rtrim( JURI::root() , '/' ) . '/components/com_myblog/templates/' . $_MY_CONFIG->get('template') . "/css/IE7.css");
		}
		...
		if(!empty($this->category))
		{
			...
			{
				$html .= "<div class=\"mbBreadcrumb\"><span class=\"filterLabel\">" . JText::_('Category') . " &raquo; </span><span class=\"filterData\">".htmlspecialchars(myGetJoomlaCategoryName($this->category)) ."</span></div>";
			}
			else
			{
				$html .= "<div class=\"mbBreadcrumb\"><span class=\"filterLabel\">" . JText::_('Tags') . " &raquo; </span><span class=\"filterData\">".htmlspecialchars(myGetTagName($this->category)) ."</span></div>";
			}
		}
	...
	function _showToolbar($op = "")
	{
		...
		if($show['feed'])
		{
			...
			if (isset ($_REQUEST['tags']) and $_REQUEST['tags'] != "")
				$rssLink .= "&category=" . htmlspecialchars($_REQUEST['tags']);
			...
			if(isset($blogger) && !empty($blogger))
			{
				...
				if($_MY_CONFIG->get('userUseFeedBurner'))
				{
					...
					$user->load(myGetAuthorId($blogger, $_MY_CONFIG->get('useFullName')));
			...
			$rssLinkHeader = '<link rel="alternate" type="application/rss+xml" title="' . $rssTitle . '" href="' . $rssLink . '" />';
                        ...
                        // not sure for this solution so I comment for temporary
		...
		if($_MY_CONFIG->get('frontpageToolbar'))
		{
			...
			$myItemid = empty($myItemid) ? $dashboardItemid:$myItemid;
			...
			if(!class_exists('AzrulJXTemplate')){
				$azbotpath = (JVERSION >= 1.6) 
						? JPATH_PLUGINS . DS . 'system' . DS . 'azrul.system' 
						: JPATH_PLUGINS . DS . 'system' ;
				include_once( $azbotpath. DS . 'pc_includes' . DS . 'template.php' );
			}
			...
			if ($op == "userblog")
			{
				...
				$manageBlogLink 		= JRoute::_("index.php?option=com_myblog&no_html=1&admin=1&task=adminhome&Itemid=$Itemid&keepThis=true&TB_iframe=true&height=600&width=850");
				$toolbar['homeLink'] 	= $homeLink;
			...
			if( !empty( $blogger ) )
			{
				$title	= stripslashes(myGetAuthorTitle(myGetAuthorId($blogger, $_MY_CONFIG->get('useFullName'))));
				$desc	= stripslashes(myGetAuthorDescription(myGetAuthorId($blogger, $_MY_CONFIG->get('useFullName'))));	
			}
