...
require_once( MY_COM_PATH . DS . 'table' . DS . 'blogcontent.php' );
...
class MyblogBrowseBase extends MyblogBaseController
{
	...
	function display()
	{
		...
		$my		=& JFactory::getUser();		
		myAddPageTitle( JText::_("COM_MY_ALL_BLOG_ENTRIES_TITLE") );
		...
		$servicelink = "<link rel='service' type='application/atomsvc+xml' href='".$base."index.php?option=com_myblog&task=atomservice' />";
		$document =& JFactory::getDocument();
		$document->addCustomTag($servicelink);
	...
	function _prepareData(&$row, $key)
	{
		...
		$row->created		= $date->toFormat();
                ...
                // check is the entry has tag {readmore} or not
                $row->checkTagReadMore = $row->fulltext && trim($row->fulltext) != ''? '1' : '0' ;

                // Add readmore link if necessary
                // also check for condition useIntrotext(1) or entry has introtext
		$row->readmore	= ($_MY_CONFIG->get('useIntrotext') == '1' || $row->checkTagReadMore=='1') ? '1' : '0';
                ...
                if($_MY_CONFIG->get('necessaryReadmore') == '1' && $row->readmore == '1')
		{
			if($row->introtext && empty($row->fulltext) )
			{
				...
				$count = myblogBlogContent::getParagraphCount($row->introtext);
			...
			else if( empty($row->introtext) && $row->fulltext )
			{
				...
				// to determine whether to display the read more link.
				$count = myblogBlogContent::getParagraphCount($row->fulltext);
        ...
        MyblogBlogContent::getBrowseText($row);
		
		//Blog comment
		$commentsys = strtolower($_MY_CONFIG->get('useComment'));
		if(!empty($commentsys)){
		
			//Might need to do as below later. JomComment need to be moved as one of the handlers in comments/:
			/*
			require_once(MY_LIBRARY_PATH .DS . 'comments.php');
            $commentSystem = new MYComments();
			$row->comments = $commentSystem->getCount(..);
			*/
			...
			switch($commentsys){
				case '1':
				case 'jomcomment' : 
					$row->comments = myCommentsURLGet($row->id, true);
					break;
				case 'intensedebate' : 
					$row->comments = $handler->getCount($row->id);
					break;
				case 'disqus' : 
					//[permalink] => /myblog/blog/baba-blacksheep.html
					$row->comments = $handler->getCount($row->id, $row->permalink);
					break;
                                case 'facebook' :
					//[permalink] => /myblog/blog/baba-blacksheep.html
                                        $row->comments = $handler->getCount($row->permalink);
					break;
				default:
					$row->comments = '';
			}
		...
		$row->rating_count	= !$row->rating_count ? 0 : $row->rating_count;
	...
	function _getEntries(&$searchby)
	{
		...
		if (!empty ($category) && empty($jcategory))
		{
			...
			$use_tables .= ",#__myblog_categories as bcat,#__myblog_content_categories as c ";
			$searchWhere .= " AND (bcat.id=c.category AND c.contentid=a.id AND bcat.id IN ($categoriesList)) ";
		}
		...
		$query = " SELECT count(*) FROM #__content as a,#__myblog_permalinks as p $use_tables WHERE a.state=1 and a.publish_up < '" .$date->toMySQL() . "' $whereSection and a.id=p.contentid $searchWhere";
		//echo $query;
		$db->setQuery($query);
		$total = $db->loadResult();
		...
		$query = "SELECT a.*, b.is_quickpost, b.quickpost_type, round(r.rating_sum/r.rating_count) as rating, r.rating_count $selectMore 
			FROM (#__content as a $use_tables ) 
				left join #__myblog_entry_attr as b ON a.id=b.contentid 
