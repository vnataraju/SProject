...
class myblogTag extends JTable
{
	...
	function setSlug($slug)
	{
		...
		$strSQL	= "SELECT COUNT(*) FROM ".$db->nameQuote('#__myblog_categories')." WHERE ".$db->nameQuote('slug')."=".$db->Quote($newslug);
		$db->setQuery( $strSQL );
	...
	{
		$db		=& JFactory::getDBO();
		$strSQL	= "SELECT COUNT(*) FROM ".$db->nameQuote('#__myblog_categories')." WHERE ".$db->nameQuote('name')."=".$db->Quote($name);
		$db->setQuery( $strSQL );
		
		if($db->loadResult() <= 0)
		{
			$strSQL	= "UPDATE ".$db->nameQuote('#__myblog_categories')." SET ".$db->nameQuote('name')."=".$db->Quote($name)." WHERE ".$db->nameQuote('id')."=".$db->Quote($this->id);
			$db->setQuery( $strSQL );
			$db->query();
			$this->name = $name;
