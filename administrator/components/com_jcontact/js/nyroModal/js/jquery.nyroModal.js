 ...
 *
 * $Date: 2008-04-30 (Wed, 30 Apr 2008) $jq
 * $jqversion: 1.2.4
 */
jQuery(function($jq) {
	...
	$jq.fn.nyroModal = function(settings) {
	...
	$jq.fn.nyroModalManual = function(settings) {
		if (!this.length)
	...
	$jq.nyroModalSettings = function(settings, deep1, deep2) {
	...
	function processModal(settings) {
		...
		if (from) {
			if (type == 'form') {
				...
				var data = $jq(from).serializeArray();
				data.push({name: currentSettings.formIndicator, value: 1});
				if (currentSettings.selector)
					data.push({name: currentSettings.selIndicator, value: currentSettings.selector.substring(1)});
				$jq.ajax(jQuery.extend({}, currentSettings.ajax, {
						url: url,
						data: data,
						type: from.method,
						success: ajaxLoaded,
				...
				var url = from.action.substring(0, from.action.length - currentSettings.selector.length);
				from.action = url;
				$jq(from).prepend('<input type="hidden" name="'+currentSettings.formIndicator+'" value="1" />');
				if (currentSettings.selector)
					$jq(from).prepend('<input type="hidden" name="'+currentSettings.selIndicator+'" value="'+currentSettings.selector.substring(1)+'" />');
			...
			initModal();
			modal.tmp.html($jq('<div/>').html(currentSettings.content).contents());
			if (modal.tmp.html())
				modal.dataReady = true;
			else
				loadingError();
	...
	function setDefaultCurrentSettings(settings) {
		...
		currentSettings = jQuery.extend({}, $jq.fn.nyroModal.settings, settings);
	...
	function initModal() {
		...
		if (!modal.full) {
			...
			if (isIE6) {
				...
				iframeHideIE = $jq('<iframe id="nyroModal"></iframe>')
								.css(jQuery.extend({},
			...
			body.append($jq('<div id="nyroModalFull"><div id="nyroModalBg"></div><div id="nyroModalWrapper"><div id="nyroModalContent"></div></div><div id="nyrModalTmp"></div><div id="nyroModalLoading"></div></div>').hide());
			
			modal.full = $jq('#nyroModalFull').show();
			modal.bg = $jq('#nyroModalBg')
				.css(jQuery.extend({
			...
			modal.loading = $jq('#nyroModalLoading')
				.css(currentSettings.css.loading)
				.hide();
			modal.contentWrapper = $jq('#nyroModalWrapper')
				.css(currentSettings.css.wrapper)
				.hide();
			modal.content = $jq('#nyroModalContent');
			modal.tmp = $jq('#nyrModalTmp').hide();
			
			// To stop the mousewheel if the the plugin is available
			if (jQuery.isFunction($jq.fn.mousewheel)) {
				modal.content.mousewheel(function(e, d) {
					if ($jq(e.target).parents('#nyroModalContent').length == 1) {
			...
			$jq(document).keydown(keyHandler);
	...
	function keyHandler(e) {
		if (e.keyCode == 27) {
			...
			if (e.keyCode == 39 || e.keyCode == 40) {
				e.preventDefault();
				$jq('.nyroModalNext', modal.content).eq(0).trigger('click');
				return false;
			} else if (e.keyCode == 37 || e.keyCode == 38) {
				e.preventDefault();
				$jq('.nyroModalPrev', modal.content).eq(0).trigger('click');
				return false;
			}
		}
	}
	...
	function fileType() {
		...
		if (from) {
			...
			var image = new RegExp('[^\.]\.('+currentSettings.extImg+')\s*$jq', 'i');
	...
	function loadingError() {
		...
		$jq(currentSettings.closeSelector, modal.loading).click(removeModal);
	}
	...
	function fillContent() {
		...
		$jq(currentSettings.closeSelector, modal.contentWrapper).click(removeModal);
		$jq(currentSettings.openSelector, modal.contentWrapper).nyroModal(currentSettings);
	}
	...
	function wrapContent() {
		...
		if (currentSettings.type == 'gallery') {
			...
			if (currentIndex > 0) {
				...
				$jq('.nyroModalPrev', modal.contentWrapper).remove();
			}
			if (currentIndex < gallery.length-1) {
				var linkNext = gallery.eq(currentIndex+1);
				$jq('.nyroModalNext', modal.contentWrapper)
				...
				$jq('.nyroModalNext', modal.contentWrapper).remove();
			}
		}
	...
	function calculateSize(resizing) {
		...
		if (currentSettings.resizable) {
			...
			if (tmp.content.height > maxHeight || tmp.content.width > maxWidth) {
				...
				if (currentSettings.type == 'image' || currentSettings.type == 'gallery') {
					...
					$jq('img#nyroModalImg', modal.content).css({
						height: calcH+'px',
						width: calcW+'px'
					});
		...
		if (!resizing) {
			...
			if (currentSettings.type == 'image' || currentSettings.type == 'gallery') {
				...
				var title = $jq('img', modal.content).attr('alt');
				$jq('img', modal.content).removeAttr('alt');
				if (title != currentSettings.defaultImgAlt) {
					var divTitle = $jq('<div>'+title+'</div>');
	...
	function ajaxLoaded(data) {
		...
		modal.tmp.html(currentSettings.selector
			?$jq('<div>'+data+'</div>').find(currentSettings.selector).contents()
			:data);
		if (modal.tmp.html()) {
			modal.dataReady = true;
			showContentOrLoading();
	...
	function formDataLoaded() {
		...
		$jq('input[name='+currentSettings.formIndicator+']', currentSettings.from).remove();
	...
	function endRemove() {
		debug('endRemove');
		$jq(document).unbind('keydown', keyHandler);
		modal.anim = false;
		modal.full.remove();
		modal.full = null;
		if (isIE6) {
	...
	function endShowContent() {
		...
		fixFF = jQuery.browser.mozilla && currentSettings.type != 'gallery' && currentSettings.type != 'image';
		if (fixFF)
	...
	function getCurCSS(elm, name) {
		var ret = parseInt($jq.curCSS(elm, name, true));
		if (isNaN(ret))
			ret = 0;
		return ret;
	}
	...
	$jq($jq.fn.nyroModal.settings.openSelector).nyroModal();	
});